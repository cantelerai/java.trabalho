/* ========= PERSISTÊNCIA ========= */
const STORE_KEY = "igrejaAppData_v1";
function loadStore(){
  const raw = localStorage.getItem(STORE_KEY);
  if(!raw){
    const seed = {
      eventos: [
        { id: 1, titulo: "Culto de Jovens", data: "2025-08-25", pregador: "Pr. João" },
        { id: 2, titulo: "Ensaio do Louvor", data: "2025-08-27", pregador: "Min. Maria" }
      ],
      membros: [
...
qs("#loginBtn").addEventListener("click", ()=>{
  ...
  qs("#tab-membros-btn").style.display = (role==="lider") ? "inline-flex" : "none";
  // selecionar tab padrão
  selectTab("tab-eventos");
  renderTudo();
...
function renderMinhaEscala(){
  ...
  if(!id){
    ...
    info.textContent = "Seu nome não está cadastrado como membro. Um líder pode te adicionar.";
